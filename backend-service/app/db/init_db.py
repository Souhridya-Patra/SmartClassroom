...
def init():
    ...
    if not admin_conn:
        raise RuntimeError(f"Unable to connect to MySQL during init: {last_error}")
    ...
    try:
        cursor.execute(
            ...
            CREATE TABLE IF NOT EXISTS class_sessions (
                ...
                timetable_section_id BIGINT NULL,
                timetable_slot_id BIGINT NULL,
                timetable_room_name VARCHAR(128) NULL,
                timetable_subject_name VARCHAR(128) NULL,
                timetable_teacher_name VARCHAR(128) NULL,
                schedule_mode VARCHAR(16) NOT NULL DEFAULT 'manual',
                ...
                INDEX idx_cs_timetable_schedule (timetable_schedule_id),
                INDEX idx_cs_status (status)
            )
            """
        )
    except Exception:
        # Hackathon-friendly fallback: keep service alive even when existing schema has FK collation drift.
        cursor.execute(
            """
            CREATE TABLE IF NOT EXISTS class_sessions (
                session_id BIGINT AUTO_INCREMENT PRIMARY KEY,
    ...
    for alter_sql in [
        "ALTER TABLE class_sessions ADD COLUMN timetable_schedule_id BIGINT NULL",
        "ALTER TABLE class_sessions ADD COLUMN timetable_section_id BIGINT NULL",
        "ALTER TABLE class_sessions ADD COLUMN timetable_slot_id BIGINT NULL",
        "ALTER TABLE class_sessions ADD COLUMN timetable_room_name VARCHAR(128) NULL",
        "ALTER TABLE class_sessions ADD COLUMN timetable_subject_name VARCHAR(128) NULL",
        "ALTER TABLE class_sessions ADD COLUMN timetable_teacher_name VARCHAR(128) NULL",
        "ALTER TABLE class_sessions ADD COLUMN schedule_mode VARCHAR(16) NOT NULL DEFAULT 'manual'",
    ]:
        try:
    ...
    try:
        cursor.execute("ALTER TABLE class_sessions ADD INDEX idx_cs_timetable_schedule (timetable_schedule_id)")
    except Exception:
        pass
